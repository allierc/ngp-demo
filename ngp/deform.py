...
class HashGridDeform(nn.Module):
    """Hash encoding + MLP producing a pixel displacement, with a level window."""

    def __init__(self, encoding: dict, decoder: dict, output_scale_px=40.0):
        super().__init__()
        self.field = NGPField(n_input_dims=2, n_output_dims=2,
                              output_activation="none", **decoder, **encoding)
        self.scale = output_scale_px
    ...
    def set_level_window(self, alpha):
        self.field.encoding.set_level_window(alpha)
...
def build_model(spec: dict, device):
    ...
    elif spec["kind"] == "hash_grid":
        m = HashGridDeform(dict(spec["encoding"]), dict(spec["decoder"]),
                           spec.get("output_scale_px", 40.0))
    else:
