...
def level_montage(model, h, w, t, device, side=110, cols=4):
    ...
    field that question has no useful answer: with 16 levels the finest clears
    any per-block threshold almost everywhere, so it reports levels 13-15 in 253
    of 256 blocks and draws a 2 px mesh.  This asks the question that does have
    one -- what does each level ADD -- and the answer is legible: measured on a
    400-step fit, levels 12-15 put 48-56% of their contribution inside the discs
    against 15.5% of the frame, while levels 7-11 sit at 15-21% and are spread
    evenly.

    The first tile is the baseline the differences start from, so baseline plus
    the fifteen differences is the fit.  Each tile is sampled at its own level's
    nodes, up to the tile size, because a grid level truly cannot represent
    anything finer than its lattice.
    """
    enc = model.encoding
    ...
    def upto(k, ny, nx):
        ...
        if key not in cache:
            ...
            cache[key] = render(model, frame_coords(ny, nx, t, device), (ny, nx))
        return cache[key]
    ...
    try:
        for j, l in enumerate([levels[0] - 1] + levels):
            ...
            ny, nx = min(hs, int(r) + 1), min(ws, int(r) + 1)
            if j == 0:
                a = upto(l, ny, nx)
                rgb = (np.clip(a.cpu().numpy() / max(1e-6, float(a.abs().max())),
                               -1, 1) * 0.5 + 0.5)
                rgb = (matplotlib.colormaps["viridis"](rgb)[..., :3] * 255).astype(np.uint8)
                labels.append(f"0..{l}")
            else:
